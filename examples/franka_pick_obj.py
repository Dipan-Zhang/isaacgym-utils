...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', '-c', type=str, default='cfg/franka_pick_obj.yaml')
    ...
    table_name, franka_name, block_name, banana_name = 'table', 'franka', 'block', 'banana'
    ...
    def setup(scene, _):
        ...
        scene.add_asset(banana_name, banana, gymapi.Transform()) # we'll sample block poses later
    ...
    policy = GraspBlockPolicy(franka, franka_name, banana, banana_name)
    ...
    while True:
        ...
        banana_transforms = [gymapi.Transform(p=gymapi.Vec3(
            (np.random.rand()*2 - 1) * 0.1 + 0.5, 
            (np.random.rand()*2 - 1) * 0.2,
            cfg['table']['dims']['sz'] + cfg['block']['dims']['sz'] / 2 + 0.1
        )) for _ in range(scene.n_envs)]
        ...
        for env_idx in scene.env_idxs:
            ...
            banana.set_rb_transforms(env_idx, banana_name, [banana_transforms[env_idx]])            
